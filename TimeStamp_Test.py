...
class Decoder(QObject):
    ...
    def decode_payload_data(self, input_message, data_len):
        if data_len >= ADXL_NUM_BYTES:
            ...
            self.past_timestamp = self.current_timestamp
            self.current_timestamp = new_data.timestamp

            if self.past_timestamp > 0:
                print((self.current_timestamp - self.past_timestamp))
